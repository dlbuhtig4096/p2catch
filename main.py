...
def main():
    ...
    loop.run_until_complete(
        asyncio.gather(bot.start(token) for token, bot in bots.items())
    )

    tasks = []
    for task in farmer({bots[token].id: bots[token] for token in FARMER}): tasks.append(task)
    for task in helper({bots[token].id: bots[token] for token in HELPER}): tasks.append(task)
    for task in player({bots[token].id: bots[token] for token in PLAYER}): tasks.append(task)
